...
function spell_parsedata($spellinfo, $buff = 0)
{
	...
	{
		$thisDuration = mysql_result(execute_query("SELECT `durationValue` FROM `dbc_spellduration` WHERE `id` = ".$spellinfo["ref_spellduration"]." LIMIT 1"), 0)/1000;
		if($thisDuration > 60)
	...
	{
		$radius = mysql_result(execute_query("SELECT `yards_base` FROM `dbc_spellradius` WHERE `id` = ".$spellinfo["ref_spellradius_1"]." LIMIT 1"), 0);
	...
	for($i = 1; $i <= 3; $i ++)
	{
		...
		if(preg_match('/\$/', $spellText))
		{
			...
			foreach($switches as $search => $table_column)
			{
				if(preg_match($search, $spellText, $matches))
				{
					$triggerdata = mysql_result(execute_query("SELECT `".$table_column."` FROM `dbc_spell` WHERE `id` = ".$matches["spellid"]." LIMIT 1"), 0);
					switch($search)
					{
						...
						case '/\${\$(?P<spellid>\d+)m'.$i.'\/10}/':
							...
							break;
						case '/\$(?P<spellid>\d+)d/':
							$thisTriggeredDuration = mysql_result(execute_query("SELECT `durationValue` FROM `dbc_spellduration` WHERE `id` = ".$triggerdata." LIMIT 1"), 0)/1000;
...
function ShowAllowable($Allowable, $table)
{
	$AllowableString = "";
	switchConnection("armory", REALM_NAME);
	$Query = execute_query("SELECT `id`, `name` FROM `".$table."`");
...
function GenerateEnchSuffixFactor($item_level, $inventory_type, $item_quality)
{
	...
	switchConnection("armory", REALM_NAME);
	return mysql_result(execute_query("SELECT `".$column."` FROM `dbc_randproppoints` WHERE `item_level` = ".$item_level." LIMIT 1"), 0);
}
