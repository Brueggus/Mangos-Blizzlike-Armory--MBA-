...
function menuInstance(menuNumber,NoOffFirstLineMenus,TargetLoc,linkClass,topLevelWidth,menuBgRef,topLevelMenuBgRef,menuBgIndentRef,underLineRef,bulletImgRef,bulletImgIndentRef,LowBgColor,topLevelLowBgColor,HighBgColor,FontLowColor,FontHighColor,BorderColor,topLevelBorderColor,BorderWidthMain,BorderWidthSub,BorderBtwnMain,BorderBtwnSub,FontFamily,FontSize,FontBold,FontItalic,MenuTextCentered,MenuCentered,MenuVerticalCentered,ChildOverlap,ChildVerticalOverlap,StartTop,StartLeft,VerCorrect,HorCorrect,DistFrmFrameBrdr,LeftPaddng,TopPaddng,FirstLineHorizontal,MenuFramesVertical,DissapearDelay,UnfoldDelay,UnfoldDelay2,TakeOverBgColor,FirstLineFrame,SecLineFrame,DocTargetFrame,MenuWrap,RightToLeft,BottomUp,UnfoldsOnClick,BaseHref,MenuUsesFrames,RememberStatus,OverFormElements,BuildOnDemand,BgImgLeftOffset,ScaleMenu,HooverBold,HooverItalic,HooverUnderLine,HooverTextSize,HooverVariant,MenuSlide,MenuShadow,MenuOpacity,beforeStartFunc,afterBuildFunc,beforeFirstOpenFunc,afterCloseAllFunc,Arrws,arrayStartLoc,arrayStartItem,topLevelName,topLevelHeight,wowcomUrl){
	...
	if(!is_ie){ //positioning adustment for nonIE browsers
		if(TargetLoc.indexOf("subMenu")>-1){ //if(arrayStartItem||TargetLoc.indexOf("subMenu")>-1){ -- Old
			StartLeft+=6;
			LeftPaddng+=6;
			topLevelWidth-=6;
		}else{
	...
	function BeforeStart(){eval(beforeStartFunc);return}
	function AfterBuild(){eval(afterBuildFunc);return}
	function BeforeFirstOpen(){eval(beforeFirstOpenFunc);return}
	function AfterCloseAll(){eval(afterCloseAllFunc);return}
...
if(document.getElementById){
	...
	if((AgntUsr.indexOf("opera")==-1||Opr7)){
		...
		FLoc,ScLoc,DcLoc,SLdAgnWin,FWinH,FColW,SColW,DColW,SWinW,SWinH,FWinW,RLvl=0,FntFc=1,StaticPos=0,Ztop=999999,FrstCreat=true,Ldd=false,
		...
		InitLdd,MstSz=0;LdTmr="";//eval('LdTmr=setInterval("menuInstanceArray['+(menuNumber-1)+'].ChckInitLd()",100)')

		}}

var menuContainer;
var menuOverCounter=0;

this.menuOverCounter=menuOverCounter;
this.LdTmr=LdTmr;
this.CurOvr=CurOvr;
this.ChckInitLd=ChckInitLd;

function ChckInitLd(){
	InitLdd=ExpYes?
		MenuUsesFrames?Par.document.readyState=="complete"&&Par.document.readyState=="complete"?
		1:0:Par.document.readyState=="complete"?
		1:0:MenuUsesFrames?Par.document.body&&Par.document.body?1:0:Par.document.body?1:0;
	if(InitLdd && (menuCookie[menuNumber-1]==1 || !MenuUsesFrames))	{clearInterval(LdTmr);Go()}} //Controls selective initial loading;
...
this.getMenuOverCounter=getMenuOverCounter;
function getMenuOverCounter(){

	return menuOverCounter;
...
function UnLdd(){
	...
	LdTmr=setInterval("menuInstanceArray["+(menuNumber-1)+"].ChckLdd()",100)}
...
function Go(){
	...
	if(!Ldd){	Crtd=false;Ldd=true;
		...
		FLoc=MenuUsesFrames?parent:window;
		ScLoc=MenuUsesFrames?parent:window;
		DcLoc=MenuUsesFrames?parent:window;
		...
		if(arrayStartLoc)arrayStartLoc=arrayStartLoc+"_";
		
		if(FrstCreat||FLoc==ScLoc)FrstCntnr=CreateMenuStructure("Menu"+arrayStartLoc,NoOffFirstLineMenus,null);
		else CreateMenuStructureAgain("Menu"+arrayStartLoc,NoOffFirstLineMenus);
		SizeMenu(FrstCntnr);
		ClcRl();if(TargetLoc)ClcTrgt();ClcLft();ClcTp();
		PosMenu(FrstCntnr,StartTop,StartLeft);
		Crtd=true;
		...
		if(RememberStatus)StMnu();
		FrstCreat=false;
		if(MenuVerticalCentered=="static"&&!AcrssFrms)setInterval("menuInstanceArray["+(menuNumber-1)+"].KeepPos()",50);
		if(!ExpYes){FWinW=-1;RePos()}
		IniFlg=true;Initiate();
		try{
			if (menuCookie[menuNumber-1] == 0)
				hideMenu(menuNumber);
		}catch(err){}
...
function SizeMenu(C){
	...
	if(topLevelWidth && C.Lvl==1){C.style.width=C.mif.style.width=topLevelWidth+P_X;}else{C.style.width=C.mif.style.width=W+P_X;} //added topLevelWidth Override
	if(topLevelHeight && C.Lvl==1){C.style.height=C.mif.style.height=topLevelHeight+P_X;}else{C.style.height=C.mif.style.height=H+P_X;} //added topLevelHeight Override
	M=C.FrstMbr;
	while(M!=null){
		...
		if(topLevelWidth && M.Lvl==1){M.style.width=topLevelWidth+P_X;} else {M.style.width=W-M.lx+P_X;} //added topLevelWidth Override
		if(topLevelHeight && M.Lvl==1){M.style.height=topLevelHeight+P_X;} else {M.style.height=H-M.tx+P_X;} //added topLevelHeight Override
		M.style.paddingLeft=M.ltp+P_X;M.style.paddingTop=M.tpd+P_X;
		if(M.ro){
			RI=(M.Lvl==1?FLoc:ScLoc).document.images[M.rid];
			if(topLevelWidth && M.Lvl==1){RI.style.width=topLevelWidth+P_X;} else {RI.style.width=W+P_X;} //added topLevelWidth Override
			if(topLevelHeight && M.Lvl==1){RI.style.height=topLevelHeight+P_X} else {RI.style.height=H+P_X} //added topLevelHeight Override		
		}
...
function HiliteItem(P){
	...
	else{	if(P.Arr[7])P.style.backgroundColor=HighBgColor;if(P.Arr[9]){P.style.color=P.Arr[9];if(P.childNodes[0].style)P.childNodes[0].style.color=P.Arr[9];}//added code to change <a> color
...
function LowItem(P){
	...
	else{	if(P.Lvl==1)P.style.backgroundColor=topLevelLowBgColor;else P.style.backgroundColor=LowBgColor;if(P.Arr[8]){P.style.color=P.Arr[8];if(P.childNodes[0].style)P.childNodes[0].style.color=FontLowColor;}//added code to change <a> color
...
try{menuClear(menuNumber);}catch(err){}
	if(Ldd&&Crtd){
		
		//document.getElementById("currOverDiv").innerHTML=this.Arr[0] + " - 1";
		CurOvr=this;IniFlg=false;ClrAllChlds(this.Ctnr.FrstMbr);HiliteItem(this);status=this.Arr[16];onBool=true;
		
		//setTimeout("OpenMenuII(CurOvr)",UnfoldDelay2);
		intervalStarter(CurOvr);
...
var intervalNotLocked=true;
//this.intervalNotLocked=intervalNotLocked;
var onBool = false;
var intervalHolder;

function intervalStarter(thisItem)
{
  countdownTimer = 0;												//Reset the timer (well, the counter)
  theTargetMenuItem = thisItem;										//this is the menu item for which we want the menu tree to be built
  if(intervalNotLocked)												//if we don't already have a countdown running
  {	
  	//document.getElementById("currOverDiv").innerHTML=thisItem.Arr[0] + " - 2";
    intervalNotLocked = false;										//lock the interval creating bit of code when you leave the door
    intervalHolder = window.setInterval("menuInstanceArray["+(menuNumber-1)+"].finalCountdown()",100);		//create the interval, calling the function finalCountdown every 10 milliseconds
  }
}
...
function finalCountdown()
{
  ...
  if (countdownTimer == 1 && onBool == true)											//after 50 clicks
  {
  	
    countdownTimer = 0;												//reset the timer
	intervalNotLocked = true;										//unlock the interval creating bit of code when you leave the door
	window.clearInterval(intervalHolder);							//clear the interval
	OpenMenuII(theTargetMenuItem);			//call the menu building function
...
function OpenMenuII(Ptr){
		//document.getElementById("currOverDiv").innerHTML=Ptr.Arr[0] + " - 3";
		...
		if(Ptr.CCn!=null&&!Ptr.CCn.Shw)RememberStatus?Unfld():OpnTmr=setTimeout("menuInstanceArray["+(menuNumber-1)+"].Unfld()",UnfoldDelay);
		status=Ptr.Arr[16]
...
function CloseMenu(){
	if(Ldd&&Crtd){
		...
		if(this==CurOvr){
			...
			IniFlg=true;CloseTmr=setTimeout("menuInstanceArray["+(menuNumber-1)+"].Initiate(menuInstanceArray["+(menuNumber-1)+"].CurOvr)",DissapearDelay)}}}
...
function MbrSetUp(MbC,PrMmbr,WMu){
	var Lctn=RLvl==1?FLoc:ScLoc,t,a,ta2=this.Arr[2];
	
	if(topLevelName && RLvl==1)
		Tfld=topLevelName;
	else
		Tfld=this.Arr[0];
	...
	if(RLvl==1){//if these are top level elements use the topLevel styles
		if(topLevelLowBgColor)this.style.backgroundColor=topLevelLowBgColor;
	}else if(this.Arr[6]&&!this.ro)this.style.backgroundColor=LowBgColor;
	
	if(ta2){	
		if(ta2.indexOf("rollover")==-1)
			if(RLvl==1 && topLevelMenuBgRef){
				if(is_ie)Tfld="<img src = /shared/wow-com/images/layout/pixel.gif width=6 height=1/>"+Tfld;
				this.style.backgroundImage="url(\""+topLevelMenuBgRef+"\")";
			}else if(RLvl==1 && topLevelName){
				//do nothing
			}else{
				if(is_ie && ta2=="reg")
					Tfld=bulletImgRef+Tfld+underLineRef; //Add in bullet and underline for IE browser.
				else if(is_ie && ta2=="indent")
					Tfld=bulletImgIndentRef+Tfld+underLineRef;
				else if (ta2=="indent"){
					Tfld="<img src = /shared/wow-com/images/layout/pixel.gif width=6 height=1/>"+Tfld;
					this.style.backgroundImage="url(\""+menuBgIndentRef+"\")";
				}else
					this.style.backgroundImage="url(\""+menuBgRef+"\")";
			}

		else{	
			this.rob=1;this.rib1=ta2.substring(ta2.indexOf("?")+1,ta2.lastIndexOf("?"));
	...
	if(this.Arr[1].charAt(0)=='default.htm'){ //add the full absolute path if it's a relative link
		this.Arr[1]=wowcomUrl+this.Arr[1];
	}	

	if(this.Arr[1].indexOf("javascript:")>-1 || !this.Arr[1])
		this.innerHTML=Tfld;//do not duplicate the link if it is a javascript call
	else
		this.innerHTML="<a href='"+this.Arr[1]+"' class='"+linkClass+"'>"+Tfld+"</a>"; //changed to support tabbed browsing
...
function CreateMenuStructure(MNm,No,Mcllr){
	RLvl++;
	if(!arrayStartItem)arrayStartItem=1;
	//alert(MNm + " " + arrayStartItem); 
	var i,Mbr,PMb=null,WMnu=MNm+arrayStartItem,MWd=eval(WMnu+"[5]")*FntFc,MHt=eval(WMnu+"[4]")*FntFc,
	...
	MbC.mif=MIF;
	MbC.SetUp=CntnrSetUp;MbC.SetUp(No,MNm+arrayStartItem,Mcllr);
	
	for(i=1;i<No+1;i++){
		WMnu=MNm+eval(i+(arrayStartItem-1));
...
var leftSideMenuArrw=["shared/wow-com/images/subnav/tri.gif",14,15,"shared/wow-com/images/layout/pixel.gif",14,15,"shared/wow-com/images/layout/pixel.gif",14,15,"shared/wow-com/images/layout/pixel.gif",14,15]
var subNavMenuArrw=['shared/wow-com/images/subnav/tri.gif',14,15,'shared/wow-com/images/subnav/arrow_right2.gif',18,12,'shared/wow-com/images/subnav/arrow_right2.gif',5,10]

var menuInstanceArray = new Array;


//if(!is_opera && !is_safari)
addEvent(window, 'load', menuGo);
addEvent(window, 'load', refreshCheck);
...
function menuClear(menuNum){
	...
	for(count=1;count<=menuInstanceArray.length;count++)
		if(menuNum!=count){menuInstanceArray[count-1].Initiate();menuInstanceArray[count-1].menuOverCounter=0;}
...
function refreshCheck(){
	...
	window.setTimeout("rePosition()",1000)
...
function rePosition(){
	...
	for(var count=1; count<=menuInstanceArray.length; count++){
			//alert(menuInstanceArray[count-1].getMenuOverCounter())
			if(!menuInstanceArray[count-1].getMenuOverCounter())
				menuInstanceArray[count-1].RePos();
	}

	/*
		for(var count=1; count<=menuInstanceArray.length; count++) {
			if(menuCookie[count-1]==1 || count > 8 )
				menuInstanceArray[count-1].RePos();
		}
	*/
...
function menuGo(){
	...
	for(count=0;count<=menuInstanceArray.length-1;count++){
		try{
		if(menuCookie[count]==1 || count>7 || menuInstanceArray.length<=7)
			menuInstanceArray[count].Go();
		}catch(err){
			menuInstanceArray[count].Go();
		}
	}
