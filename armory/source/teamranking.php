...
<option value="index.php?searchType=team&type=2&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">2v2</option>
<option value="index.php?searchType=team&type=3&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">3v3</option>
<option value="index.php?searchType=team&type=5&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">5v5</option>
...
<option selected value="#"></option>
<option value="index.php?searchType=team&type=<?php echo $type,"&realm=",REALM_NAME ?>&sortBy=wins2"><?php echo $lang["wins"] ?></option>
<option value="index.php?searchType=team&type=<?php echo $type,"&realm=",REALM_NAME ?>&sortBy=losses"><?php echo $lang["losses"] ?></option>
<option value="index.php?searchType=team&type=<?php echo $type,"&realm=",REALM_NAME ?>&sortBy=rating"><?php echo $lang["rating"] ?></option>
</select></em>
</div>
</div>
</div>
...
$pvpquery = execute_query("SELECT at.`arenateamid`, at.`name`, `BackgroundColor`, `EmblemStyle`, `EmblemColor`, `BorderColor`, `rating`, `wins2`, `played` - `wins2` AS `losses`, `rank`, c.`name` AS captain_name, `race`
FROM `arena_team` AS at, `arena_team_stats` AS atm, `characters` AS c
WHERE at.`arenateamid` = atm.`arenateamid` AND `played` <> 0 AND `type` = ".$type." AND `captainguid` = c.`guid`".exclude_GMs().
" ORDER BY `".$orderField."` DESC LIMIT ".$config["TeamTop"]);
$i = 0;
while($team = mysql_fetch_assoc($pvpquery))
{
	$i ++;
	$team["faction"] = GetFaction($team["race"]);
	$team["captain_name"] = $team["captain_name"] ? $team["captain_name"] : $lang["unknown"];
	$teamMembers = mysql_result(execute_query("SELECT COUNT(*) FROM `arena_team_member` WHERE `arenateamid` = ".$team["arenateamid"]), 0);
?>
<tr>
<td>
<div>
