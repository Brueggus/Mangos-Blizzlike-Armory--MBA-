...
<option value="index.php?searchType=arena&type=2&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">2v2</option>
<option value="index.php?searchType=arena&type=3&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">3v3</option>
<option value="index.php?searchType=arena&type=5&realm=<?php echo REALM_NAME,"&sortBy=",$orderField ?>">5v5</option>
...
<option selected value="#"></option>
<option value="index.php?searchType=arena&type=<?php echo $type,"&realm=",REALM_NAME ?>&sortBy=personal_rating"><?php echo $lang["rating"] ?></option>
<option value="index.php?searchType=arena&type=<?php echo $type,"&realm=",REALM_NAME ?>&sortBy=arenapoints"><?php echo $lang["points"] ?></option>
</select></em>
</div>
</div>
</div>
...
if($orderField == "arenapoints")
	$tablefield = "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`data`, ' ', ".($defines["ARENAPOINTS"][CLIENT]+1)."), ' ', -1) AS UNSIGNED)";
else//if($orderField == "personal_rating")
	$tablefield = "`personal_rating`";
switchConnection("characters", REALM_NAME);
$pvpquery = execute_query("SELECT c.`guid`, `data`, c.`name`, `race`, `class`, atm.`arenateamid`, `personal_rating`, at.`name` AS arena_team_name, `captainguid`
...
while($char = mysql_fetch_assoc($pvpquery))
{
	...
	$char["arenapoints"] = $char_data[$defines["ARENAPOINTS"][CLIENT]];
	...
	{
		switchConnection("characters", REALM_NAME);
		if($teamCaptain = mysql_fetch_assoc(execute_query("SELECT `name` FROM `characters` WHERE `guid` = ".$char["captainguid"]." LIMIT 1")))
			$teamCaptainName = $teamCaptain["name"];
		else
			$teamCaptainName = $lang["unknown"];
		$teamMembers = mysql_result(execute_query("SELECT COUNT(*) FROM `arena_team_member` WHERE `arenateamid` = ".$char["arenateamid"]), 0);
