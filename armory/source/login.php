...
if(isset($_POST["name"]) && isset($_POST["pass"]) && isset($_POST["realm"]) && isset($realms[$realm_name = urldecode($_POST["realm"])]))
{
	...
	else
	{
		...
		$user_pass = sha1(strtoupper($user_name.":".stripslashes($_POST["pass"])));
		$results = execute_query("SELECT `id`, `username`, `gmlevel` FROM `account` WHERE `username` = '".mysql_real_escape_string($user_name)."' AND `sha_pass_hash` = '".$user_pass."' LIMIT 1");
...
else
{
	...
	{
		execute_query("UPDATE `account` SET `expansion` = '".$_POST["expansion"]."' WHERE `id` = ".$_SESSION["user_id"]." LIMIT 1");
		$exp_changed = 1;
		$expansion = $_POST["expansion"];
	}
	else
		$expansion = mysql_result(execute_query("SELECT `expansion` FROM `account` WHERE `id` = ".$_SESSION["user_id"]." LIMIT 1"), 0);
	$selected_0 = "";
	$selected_1 = "";
	$selected_2 = "";
	switch($expansion)
...
<?php
	...
	$squery = execute_query("SELECT `name` FROM `characters` WHERE `account` = ".$_SESSION["user_id"]);
