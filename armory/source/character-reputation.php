...
<p>".GetNameFromDB($key, "dbc_faction")."</p>
...
<h1><?php echo $lang["reputation"] ?></h1>
</div>
<?php
switchConnection("characters", REALM_NAME);
$return_char_reputations = execute_query("SELECT `faction`, `standing` FROM `character_reputation` WHERE `guid` =".$stat["guid"]." AND (`flags` & 1 = 1)");
while($factions = mysql_fetch_assoc($return_char_reputations))
{
	$standing = $factions["standing"];
	switchConnection("armory", REALM_NAME);
	$faction_info = mysql_fetch_assoc(execute_query("SELECT * FROM `dbc_faction` WHERE `id` =".$factions["faction"]." LIMIT 1"));
