...
if($do_query)
{
	...
	$mlquery = execute_query("SELECT * FROM `characters`, `arena_team_member` WHERE `characters`.`guid`=`arena_team_member`.`guid` and `arenateamid` = ".$arenateam["arenateamid"].exclude_GMs()." ORDER BY `personal_rating` DESC");
	// Get number of members in arenateam //
	$arenateam_members = mysql_num_rows($mlquery);
	// Statistics //
	if($arenateam["wins"])
		$arenateam["win_percent_week"] = round(($arenateam["wins"] / $arenateam["games"]) * 100);
	else
		$arenateam["win_percent_week"] = 0;
	if($arenateam["wins2"])
		$arenateam["win_percent_season"] = round(($arenateam["wins2"] / $arenateam["played"]) * 100);
	else
		$arenateam["win_percent_season"] = 0;
	$arenateam["losses_week"] = $arenateam["games"] - $arenateam["wins"];
	$arenateam["losses_season"] = $arenateam["played"] - $arenateam["wins2"];
...
<div id="teamicon<?php echo $arenateam["type"] ?>" style="display:none;"></div>
<script type="text/javascript">
...
<p><?php echo $arenateam["games"] ?></p>
</td><td align="center">
<p><?php echo $arenateam["wins"]," - ",$arenateam["losses_week"] ?></p>
</td><td align="center">
...
<p><?php echo $arenateam["played"] ?></p>
</td><td align="center">
<p><?php echo $arenateam["wins2"]," - ",$arenateam["losses_season"] ?></p>
</td><td align="center">
...
<?php
	while($cdata = mysql_fetch_assoc($mlquery))
	{
