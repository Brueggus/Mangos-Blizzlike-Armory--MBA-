...
else
{
	...
	$itemQuery = execute_query("SELECT `ItemLevel`, `BuyPrice`, `SellPrice`, `RequiredDisenchantSkill`, `RandomProperty`, `RandomSuffix`, `ItemLevel`, `InventoryType`, `DisenchantID`
FROM `item_template` WHERE `entry` = ".$item_ID." LIMIT 1");
	if(!($itemData = mysql_fetch_assoc($itemQuery)))
...
else
{
	...
	$item_query = execute_query("SELECT `item_icon`, `item_quality` FROM `cache_item` WHERE `item_id` = ".$item_ID." AND `mangosdbkey` = ".$realms[REALM_NAME][2]." LIMIT 1");
	if(!($item_info = mysql_fetch_assoc($item_query)))
	{
	...
	switchConnection("armory", REALM_NAME);
	$item_query = execute_query("SELECT `item_info_html` FROM `cache_item_tooltip` WHERE `item_id` = ".$item_ID." AND `mangosdbkey` = ".$realms[REALM_NAME][2]." LIMIT 1");
	if(!($item_info = mysql_fetch_assoc($item_query)))
	{
...
/******************************************************************************************************************************************************************/
// Random Enchantment
	if($config["ShowRandomEnch"] && ($itemData["RandomProperty"] || $itemData["RandomSuffix"]))
	{
		...
		switchConnection("mangos", REALM_NAME);
		$RandomQuery = execute_query("SELECT * FROM `item_enchantment_template` WHERE `entry` = ".$randomentry);
		if(mysql_num_rows($RandomQuery))
		{
?>
...
<?php
			...
			{
				$enchantment = mysql_fetch_assoc(execute_query("SELECT * FROM `".$randomtable."` where `id` = ".$Random["ench"]." LIMIT 1"));
				$enchantmentbonus="";
				for($i = 1; $i <= 3; $i ++)
				{
					if($enchantment["ref_spellitemenchantment_".$i])
					{
						$enchantmentbonus_name = mysql_result(execute_query("SELECT `name` FROM `dbc_spellitemenchantment` where `id` = ".$enchantment["ref_spellitemenchantment_".$i]." LIMIT 1"), 0);
						if($enchantmentbonus)
							$enchantmentbonus .=", ";
...
// Disenchant
	...
	{
		if($itemData["RequiredDisenchantSkill"] > -1 && $itemData["DisenchantID"])
		{
			switchConnection("mangos", REALM_NAME);
			$DisenchantLootQuery = execute_query("SELECT * FROM `disenchant_loot_template` WHERE `entry` = ".$itemData["DisenchantID"]);
			if(mysql_num_rows($DisenchantLootQuery))
			{
?>
<div class="scroll-padding"></div>
<div class="rel-tab">
...
<?php
			...
			else if($itemData["RequiredDisenchantSkill"] > -1 && $config["ShowError"])
