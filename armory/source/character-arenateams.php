...
foreach($arenatypes as $type)
{
	...
	$teamquery = execute_query("SELECT atm.`arenateamid`, `played_season`, `wons_season`, `personal_rating`, `played` ,`wins2`, `rating`, `rank`, at.`name`, `BackgroundColor`, `EmblemStyle`, `EmblemColor`, `BorderColor`, c.`name` AS cap_name, `race`, `class`, `data`
	 ...
	 AND atm.`guid` = ".$stat["guid"]." AND `type` = ".$type." LIMIT 1");
	if($team = mysql_fetch_assoc($teamquery))
	{
		if($team["wins2"])
		{
