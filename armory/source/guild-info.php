...
if($do_query)
{
	...
	$mlquery = execute_query("SELECT * FROM `characters`, `guild_member` WHERE `characters`.`guid`=`guild_member`.`guid` and `guildid` = ".$guild["guildid"].exclude_GMs()." ORDER BY `rank` ASC");
	// Get number of members in guild //
	$guild_members = mysql_num_rows($mlquery);
	// Faction Info //
	// Member Data //
...
<h1><?php echo $lang["guild_roster"] ?></h1>
</div>
</div>
...
<?php
	...
	while($cdata = mysql_fetch_assoc($mlquery))
	{
		...
		$i = $i + 1;
		echo "theArray[",$i," - 1] = [[\"&character=",$cdata["name"],"&realm=",REALM_NAME,"\"], [\"",$cdata["name"],"\"], [",$cdata["level"],"], [\"",$cdata["race"],"\", \"",$cdata["gender"],"\", \"",GetNameFromDB($cdata["race"], "dbc_chrraces"),"\"], [\"",$cdata["class"],"\", \"",GetNameFromDB($cdata["class"], "dbc_chrclasses"),"\"], [\"",$cdata["rank"],"\"]]; \n";
	}
?>
	var gHighestRank = 0;
